...
def init_weights(shape, stddev=0.01, name=None):
    weights = tf.Variable(tf.truncated_normal(shape, stddev=stddev), name=name)
...
class MultiClassifier(object):
    ...
    def load_tfrecord_shuffle(self, filename, graph,
    ...
    def load_tfrecord(self, filename, graph, batch_size=TEST_BATCH_SIZE):
        """
        load testing data
        """
        with graph.as_default():
    ...
    def train_single(self, filename, clf_id, num_epochs=5, gpu_option='0'):
        ...
        with graph.as_default():
            ...
            labels, features = self.load_tfrecord_shuffle(filename,
                                                        graph,
                                                        num_epochs=num_epochs)
            ...
            train_op = tf.train.AdamOptimizer(epsilon=EPSILON).minimize(loss)
